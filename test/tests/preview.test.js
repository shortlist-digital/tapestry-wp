...
describe('Handling preview requests', () => {
    ...
    before(done => {
      ...
      // mock api response
      nock('http://dummy.api')
        .get('/wp-json/revision/v1/posts?slug=preview-test&tapestry_hash=hash&p=10')
        .reply(200, dataPost)
    ...
    it('Preview route renders and is not cached', (done) => {
      ...
      cacheApi.reset()
      request.get(`${uri}/dynamic-string-endpoint/preview-test?tapestry_hash=hash&p=10`, async (err, res, body) => {
