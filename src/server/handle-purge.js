...
export default ({ server, config }) => {
  const calculateApiRoute = (path, cb) => {
    match({
      ...
      logger.debug(`Purge request for endpoint ${chalk.green(endpoint)}`)
      cb(endpoint)
    })
  }
  ...
  server.route({
    ...
    handler: (request, reply) => {
      calculateApiRoute(request.params.path, (apiRoute) => {
        ...
        logger.debug(`Request: ${chalk.green(request.params.path)} mapped to API: ${apiRoute}`)
        const remote = `${config.siteUrl}/wp-json/wp/v2/${apiRoute}`

        logger.debug(`Directly clearing html cache for ${chalk.green(request.params.path)}`)
        cacheManager.clearCache('html', request.params.path)

        logger.debug(`Directly clearing api cache for ${chalk.green(remote)}`)
